...
function field (path, options) {
	...
	function markType(options){
		...
		if (options.type.name === 'html' && options.wysiwyg) {
			listThis.fieldTypes.wysiwyg = true;
		}
		if(options.type.name === 'objectarray' && options.layoutMode == '2d'){
			listThis.fieldTypes.objectArray2dLayout = true;
		}
		if(options.type.name === 'objectarray' || options.type.name === 'nestedobject'){
			var parts = options.parts;
			_.forEach(parts, function(value, key){
				markType(value);
			});
		}
		if(options.type.name === 'code'){
			listThis.fieldTypes.code = true;
		}
	}
	markType(options);
